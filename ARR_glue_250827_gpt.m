...
%% ====================== 初始参数配置 ======================
xsize = 3295600/400;   % X轴维度尺寸（根据原始数据维度设置）
ysize = 400;    % Y轴维度尺寸
wavmin = 470;   % 波长最小值(nm)
wavmax = 560;   % 波长最大值(nm)
...
for ii = 1:n_max
    ...
    if contains(cfsj(ii).name,'WHITE') 
        if contains(cfsj(ii).name,'600') && contains(cfsj(ii).name,'Z5')
            ...
            % ================= 每个波长逐点拟合边界 =================
            pixel_l_manual = nan(xsize,1);
            pixel_r_manual = nan(xsize,1);

            intensity_norm = intensity ./ max(intensity(:));  % 归一化
            threshold = 0.05;  % 阈值：小于最大值10%的光谱跳过拟合

            for jj = 1:xsize
                spectrum = intensity_norm(jj,:);
                if max(spectrum) < threshold
                    continue;  % 信号太弱，不拟合
                end
                try
                    param = dual_logistic(pixellength(jj,30:370), intensity(jj,30:370));
                    pixel_l_manual(jj) = param.x1;
                    pixel_r_manual(jj) = param.x2;
                catch
                    % 拟合失败，跳过，保持 NaN
                    continue;
                end
            ...
            % ================= 对缺失数据插值补全 =================
            pixel_l_fit = fillmissing(pixel_l_manual,'linear','EndValues','nearest');
            pixel_r_fit = fillmissing(pixel_r_manual,'linear','EndValues','nearest');

            % 可视化拟合结果
            figure;
            plot(pixeldata, pixel_l_fit,'r-', pixeldata, pixel_r_fit,'b-');
            xlabel('Pixel index'); ylabel('Boundary position');
            legend('Left boundary','Right boundary');
            title('Step-wise boundary fitting with interpolation');
...
for ii = 1:n_max
    ...
    if isempty(findstr(cfsj(ii).name,'WHITE'))   
      if ~isempty(findstr(cfsj(ii).name,'600')) && ~isempty(findstr(cfsj(ii).name,'Z5'))
        % 数据导入与插值处理
        arplfile = importdata(cfsj(ii).name);
...
function params = dual_logistic(x, y)
    ...
    lb = [0, -Inf, min(x)+70, min(x)+250, 0, 0];  % 参数下限
    ub = [Inf, Inf, max(x)-170, max(x)-50, Inf, Inf]; % 参数上限
